...
temp_rise_vals = bottom_sig.all_temp_rise_due_to_laser_abs+15;

grueneisen_corr = 0.1125*temp_rise_vals + 6.75;
PA_sigs_corrected =combined_av_sig_vals./grueneisen_corr;
PA_sigs_corrected_abs =abs(combined_av_sig_vals)./abs(grueneisen_corr);


figure;
npfsa_h = plot(grueneisen_corr);
...
set(gca,'FontSize',16);
ylabel('Gruensiesn corr');
xlabel('Time elapsed (seconds)');


figure;
npfsa_h = plot(PA_sigs_corrected);
for line_num = 1:size(npfsa_h,1)
    set(npfsa_h(line_num),'LineStyle',line_style_array(line_num,:));
    set(npfsa_h(line_num),'Color',line_colour_array(line_num,:));
    set(npfsa_h(line_num),'LineWidth',2);
